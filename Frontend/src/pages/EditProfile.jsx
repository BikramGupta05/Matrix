        ...
        <div className='bg-white rounded-2xl shadow-lg p-8 max-w-xl w-full relative'>
            <FaArrowLeftLong className='absolute top-[5%] left-[5%] w-[22px] h-[22px] cursor-pointer' onClick={()=>navigate("/profile")}/>
                ...
                <form action="" className='space-y-5' onSubmit={(e)=>e.preventDefault()}>
                    ...
                    <div>
                        <label htmlFor="image" className='text-sm font-medium text-gray-700'>Select Image</label>
                    ...
                    <button className='w-full bg-[black] active:ng-[#454545] text-white py-2 rounded-md font-medium transition  cursor-pointer ' disabled={loading} onClick={handleEditProfile}>{loading?<ClipLoader size={30} color='grey'/>:"Save Changes"}</button>
                </form>
        </div>
    </div>
  )
